...
class Edit extends React.Component {
    ...
    .then(ret => {
      ...
      .then(resJson => {
          if(resJson.success) {
            ...
            //"http://93.188.162.83:4200/api/upload"
            RNFS.uploadFiles({
  ...
  componentDidMount() {
    ...
    .then(ret => {
      ...
      var addressArr = customerTmp.addresses;
      var leng = addressArr.length;
      addressTmp = addressArr[leng - 1];
      this.setState({street:addressTmp.address1});
      this.setState({city:addressTmp.city});
      this.setState({country:addressTmp.country});
      this.setState({province:addressTmp.province});
    });
    ...
    .then(ret => {
      var exDate = ret.exDate;
      exDate = exDate[0] + '/' + exDate[1];
      exDate = exDate != "/undefined" ? exDate : '';
      var name = ret.name;
      name = name[0] + ' ' + name[1];
      name = name != " undefined" ? name : '';
      this.setState({cardNumber:ret.cardNumber});
      this.setState({name:name});
      this.setState({securityCode:ret.securityCode});
  ...
  async _updateAddress() {
    ...
    .then( async ret => {
      ...
      // this.setState({error: JSON.stringify(json)});
              ...
              <ScrollView>              
                ...
                <View style={styles.btnGroup}>
                  <View style={styles.fBtnContainer}>
                    ...
                    {/* <TouchableWithoutFeedback  style={styles.profileImageTouch} onPress={() => this.handleChoosePhoto()}> */}
                ...
                <View style={styles.changeContainer}>
                  <View style={styles.change}>
                    ...
                    <ButtonComponent btnColor="black" btnText="SHIPPING ADDRESS" width={wp("40%")}></ButtonComponent>
                    ...
                    <View style={styles.chnageBtnContainer}>
                      ...
                      <View style={styles.changeDetail, {flex: 3}}>
                          <Text style={{fontSize: 16, marginLeft: '5%'}}>{street}</Text>
                          <Text style={{fontSize: 16, marginLeft: '5%'}}>{city}, {country}</Text>
                      </View>

                      <TouchableOpacity style={{flex: 3}} onPress={() => {this._setShippingModalShow()}}>
                        <ButtonComponent btnColor="white" btnText="CHANGE" marginLeft="14%"></ButtonComponent>
                      </TouchableOpacity>
                    </View>
                  </View>

                  <View style={styles.change}>
                    <ButtonComponent btnColor="black" btnText="PAYMENT METHOD" width={wp("40%")}></ButtonComponent>
                    ...
                    <View style={styles.chnageBtnContainer2}>
                      ...
                      <View style={styles.payNumber}>
                        {
                          card != '' ? <Text style={styles.dots}>.... .... ....</Text> : null
                        }
                        {
                          card != '' ? <Text style={styles.number}>{card}</Text> : null
                        }
                      </View>
                      <TouchableOpacity style={{flex: 2}} onPress={() => this._setPaymentModalShow()}>
                        <ButtonComponent btnColor="white" btnText="CHANGE" marginLeft="14%"></ButtonComponent>
                      </TouchableOpacity>
                    </View>
                  </View>
                ...
                <View style={styles.saveBtn}>
                  <TouchableOpacity onPress={() => this._updateProfile()}>
                    <Image  style={styles.saveBtnImage} resizeMode="contain" source={require('../images/save1.png')} />
                  </TouchableOpacity>
                </View>
                </ScrollView>           
            </View>
